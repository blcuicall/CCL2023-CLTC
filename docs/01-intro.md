...
|   |原句数|参考句数|平均参考句数|有修改的参考句数（比例）|原句平均字符数|参考局平均字符数|
| --- | --- | --- | --- | --- | --- | --- |
...
**1）** 我们首先使用基于字的编辑抽取工具抽取出预测编辑集合 $$\mathcal{e}$$ 和正确编辑集合 $$\mathcal{g}$$；
...
如果当前句子有多种修改方式（假设有$$\mathcal{n}$$种），那么我们对每个修改方式都抽取一个编辑集合，将预测编辑集合与所有正确编辑集合对比，选取尽可能大的$$F_{0.5}$$指标作为当前句子的指标。
